...
public abstract class UpdateableTask<V> extends Task<V> {
	...
	public static final EventType<WorkerStateEvent> FINISHED_EVENT = new EventType<>(WorkerStateEvent.ANY, "finished");
	...
	/**
	 * Called whenever the task stops. This could be if the task finished successfully, the task failed
	...
	public void finished () {
		if (handler != null) {
			Platform.runLater(new Runnable() {
				...
				@Override
				public void run() {
					handler.handle(new WorkerStateEvent(UpdateableTask.this, FINISHED_EVENT));	
				}
			});
		}
	}
