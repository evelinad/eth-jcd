...
public class DirectoryTree extends TreeView<String> implements
		DiskEntryListener, StateListener, DirectoryListener {
	...
	public class DirectoryTreeItem extends TreeItem<String> {
		...
		public DirectoryTreeItem(String value) {
			super(value, new ImageView(ImageResource.getImageResource()
					.folderImage()));
		}
					...
					if (entries != null) {
						for (Entry entry : entries) {
							if (entry instanceof Directory) {
								addChild(DirectoryTreeItem.this,
										new DirectoryTreeItem(entry.getPath()
												.getName()));
							}
						}
					}
				...
				new ChangeListener<TreeItem<String>>() {
					...
					public void changed(
						...
						if (selectionChangeListenerEnabled) {
							if (newValue != null) {
								Directory currentDirectory = guiState
										.getCurrentDirectory();
								Path itemPath = new Path(getPath(newValue));
								if (currentDirectory != null
										&& !currentDirectory.getPath().equals(
												itemPath)) {
									guiState.setCurrentDirectory(new Directory(
											itemPath));
								}
							}
						}
	...
	public void addEntry(Entry entry, DirectoryTreeItem newEntry) {
		...
		for (int i = 1; i < split.length - 1; i++) {
			boolean found = false;
			if (currentItem != null
					&& ((DirectoryTreeItem) currentItem).hasChildrenLoaded()) {
	...
	private void addChild(TreeItem<String> parent, TreeItem<String> child) {
		// Because the selection changes for some reason if a new child is
		// added,
		// the current selection has to be stored and set back after the child
		// was added.
	...
	public void entryChanged(Entry oldEntry, Entry newEntry) {
		...
		if (deletedEntry == null) {
			addEntry(newEntry, new DirectoryTreeItem(newEntry.getPath()
					.getName()));
		} else {
