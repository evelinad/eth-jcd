...
public class RemoteSynchronizedTaskController extends RemoteTaskController
		implements SynchronizedTaskController {
	...
	private void initialize(URI localUri, URI serverUri) {
		...
		if (serverUri != null && !isServerUri(serverUri)) {
			throw new IllegalArgumentException("Illegal server uri "
					+ serverUri);
		}
		this.uri = localUri;
	...
	private void registerServerClient() throws RemoteException {
		if (serverRmiClient == null) {
			...
			serverConnection.getDisk().registerClient(
					serverConnection.getDiskId(), serverRmiClient);
		}
	}
	...
	protected void unregisterClient() throws RemoteException {
		...
		if (serverRmiClient != null) {
			serverConnection.getDisk().unregisterClient(
					serverConnection.getDiskId(), serverRmiClient);
	...
	private void closeLocalConnection() throws RemoteException,
			VirtualDiskException {
		connection.getDisk().unloadDisk(connection.getDiskId());
		connection = null;
	}

	private void closeServerConnection() throws RemoteException,
			VirtualDiskException {
	...
	protected void checkIsLocalConnected() {
		if (!isLocalConnected()) {
			throw new IllegalStateException(
					"Controller is not connected to local disk");
		}
	}
	...
	protected void checkIsServerConnected() {
		if (!isServerConnected()) {
			throw new IllegalStateException(
					"Controller is not connected to server disk");
		}
	}
	...
	protected void updateLastSynchronized(long lastSynchronized) {
		try {
			AdditionalLocalDiskInformation localDiskInformation = RemoteConnectionUtil
					.getDiskInformation(connection);
	...
	private void saveLocalDiskInformation(
			AdditionalLocalDiskInformation diskInformation)
			throws RemoteException, VirtualDiskException {
		connection.getDisk().saveAdditionalDiskInformation(
				connection.getDiskId(), diskInformation.toByteArray());
	}
	...
	public UpdateableTask<Void> createGoOnlineTask(final String password) {
		...
		return new UpdateableTask<Void>() {
			...
			protected Void callImpl() throws RemoteException,
					...
					ConnectionException, NotBoundException, URISyntaxException,
					InterruptedException {
				checkIsLocalConnected();
				AdditionalLocalDiskInformation localDiskInformation = RemoteConnectionUtil
						.getDiskInformation(connection);
	...
	@Override
	public UpdateableTask<Void> createLinkDiskTask(final String server,
			final String userName, final String password, final String diskName) {
		checkIsLocalConnected();
		return new UpdateableTask<Void>() {
			...
			protected Void callImpl() throws RemoteException,
					...
					InterruptedException {
				checkIsLocalConnected();
				AdditionalLocalDiskInformation diskInformation = new AdditionalLocalDiskInformation(
						userName, server, diskName, 0);
	...
	public UpdateableTask<Void> createDownloadDiskTask(final URI localUri) {
		...
		return new UpdateableTask<Void>() {
			...
			protected Void callImpl() throws RemoteException,
					VirtualDiskException, AuthenticationException,
					ConnectionException, NotBoundException,
					InterruptedException {
				checkIsServerConnected();
				...
				saveLocalDiskInformation(new AdditionalLocalDiskInformation(
						userInfoSplit[0], serverUri.getHost(),
						serverUri.getQuery(), 0));
				acquireLock(this);
				try {
	...
	public void synchronizeDisks(UpdateableTask<?> task,
			RemoteConnection prevConnectedConnection) throws RemoteException,
			VirtualDiskException {
		if (!isLocalConnected() || !isServerConnected()) {
			throw new IllegalStateException(
					"Can't synchronize disks if not connected to both server and local disk");
		}
		synchronizing = true;
		try {
			SynchronizeDisks synchronizeDisks = new SynchronizeDisks(
	...
	public void entryAdded(final Entry entry) throws RemoteException,
		...
		if (synchronizeChanges()) {
			if (entry instanceof Directory) {
				connection.getDisk().createDirectory(connection.getDiskId(),
						(Directory) entry);
			} else if (entry instanceof File) {
				connection.getDisk().createFile(connection.getDiskId(),
						(File) entry);
			}
		}
	}
		...
		if (synchronizeChanges()) {
			if (oldEntry.getPath().equals(newEntry.getPath())) {
				if (oldEntry.getTimestamp() != newEntry.getTimestamp()) {
					connection.getDisk().updateLastModified(
							connection.getDiskId(), oldEntry,
							newEntry.getTimestamp());
				}
			} else {
				connection.getDisk().moveEntry(connection.getDiskId(),
						oldEntry, newEntry);
			}
		}
	}
	...
	public void entryCopied(final Entry sourceEntry,
		...
		if (synchronizeChanges()) {
			connection.getDisk().copyEntry(connection.getDiskId(), sourceEntry,
					destinationEntry);
		}
	}
	...
	public void writeToEntry(final File file, final long fileOffset,
		...
		if (synchronizeChanges()) {
			connection.getDisk().write(connection.getDiskId(), file,
					fileOffset, data);
		}
	}
}
