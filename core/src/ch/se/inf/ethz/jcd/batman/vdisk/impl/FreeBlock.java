 ...
 * block 0x08 8 byte offset position of the previous block in the list 0x10 8
 * byte offset position of the next block in the list 0xnn n garbage end 8 byte
 * size and indicator for free block
 * 
 * @see IVirtualBlock
 * @see IDataBlock
 */
public final class FreeBlock extends VirtualBlock implements IFreeBlock {
	 ...
	 *             error occurs
	 */
	public static final IFreeBlock load(IVirtualDisk disk, long position)
			throws IOException {
	 ...
	 * @param previous
	 *            the offset position of the previous block in the list
	 * @param next
	 ...
	 */
	public static final IFreeBlock create(IVirtualDisk disk, long position,
			long size, long previous, long next) throws IOException {
	...
	private FreeBlock(IVirtualDisk disk, long position, long size,
			long previous, long next) throws IOException {
	...
	private void updateNextBlock() throws IOException {
		writeRealPosition(VirtualBlock.METADATA_START_SIZE + PREVIOUS_SIZE,
				next);
	}
