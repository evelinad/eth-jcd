...
public class HostBridgeTest {
	...
	public void testImportExportDirectory() throws IOException {
		...
		VDiskFile virtualSubDir = new VDiskFile(virtualDir,
				hostSubDir.getName());
		...
		List<String> exportedHostDirChilds = Arrays
				.asList(exportHostDir.list());
		...
		List<String> exportedHostSubDirChilds = Arrays.asList(exportHostSubDir
				.list());
		assertEquals(virtualSubDirChilds.size(),
				exportedHostSubDirChilds.size());
