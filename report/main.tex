...
\begin{enumerate}
    ...
    \item start the GUI (\mono{.browser.view.Main})
...
The connection to the server is done by using a correct URI. If the server runs locally and you would like to create and save the virtual disk under \mono{/var/tmp/my.vdisk} you would use the following URI to connect (and create in the same step if not yet existent): \mono{batman://localhost?/var/tmp/my.vdisk}. If you're on a Microsoft Windows machine this would be a valid URI: \mono{batman://localhost?C:/gui.vdisk}. Therefore in general the URI is built like this: \mono{batman://[SERVER]?[FILE PATH ON SERVER SYSTEM]}.
